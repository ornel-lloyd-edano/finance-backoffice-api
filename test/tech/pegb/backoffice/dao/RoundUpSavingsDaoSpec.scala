...
class RoundUpSavingsDaoSpec extends PegBTestApp with MockFactory {
  ...
  override def initSql =
    s"""
       ...
       |(id, uuid, user_id, saving_account_id, current_amount, rounding_nearest, status_updated_at, created_at, updated_at, is_active)
       |VALUES
       |(1, 'c86eb85f-2b61-5d42-ae28-0d01f8e5de71', '1', '1', '9999', '10', '2019-07-06 08:13:00', '2019-07-04 09:13:00', '2019-07-11 07:36:46', '1'),
       |(2, 'fc3df811-adea-4e8d-9ae5-fbfc66c8b07d', '2', '2', '5000', '10', '2019-07-06 08:13:00', '2019-05-10 09:13:00', '2019-07-11 07:36:46', '0'),
       |(3, 'b80f8522-9c58-4464-b4bc-0d6ef91b612e', '3', '3', '1500', '10', '2019-07-06 08:13:00', '2019-03-06 09:13:00', '2019-07-11 07:36:46', '0'),
       |(4, 'd5698075-aa7a-40b4-834b-8f0e88f1a3b8', '4', '4', '2800', '10', '2019-07-06 08:13:00', '2019-01-01 09:13:00', '2019-07-11 07:36:46', '1');
     """.stripMargin

  override def cleanupSql: String =
    s"""
       |DELETE FROM roundup_savings;
  ...
  val roundUpSavingsDao = inject[RoundUpSavingsDao]

  "RoundUpSavingsDao" should {
    "get all RoundUpSaving records in getSavingOptionsByCriteria" in {

      val result = roundUpSavingsDao.getSavingOptionsByCriteria(None, None, None, None)
    ...
    "count all RoundUpSaving records in countSavingOptionsByCriteria" in {
      val result = roundUpSavingsDao.countSavingOptionsByCriteria(None)
      result.right.get mustBe 4
    }

    "get filtered RoundUpSaving records in getSavingOptionsByCriteria" in {

      val filterById = RoundUpSavingsCriteria(id = Option(CriteriaField("", 3)))
      val result1 = roundUpSavingsDao.getSavingOptionsByCriteria(Option(filterById), None, None, None)

      val filterByUuid = RoundUpSavingsCriteria(uuid = Option(CriteriaField("", "c86eb85f-2b61-5d42-ae28-0d01f8e5de71")))
      val result2 = roundUpSavingsDao.getSavingOptionsByCriteria(Option(filterByUuid), None, None, None)

      val filterByCurrency = RoundUpSavingsCriteria(currency = Option(CriteriaField("", "AED")))
      val result3 = roundUpSavingsDao.getSavingOptionsByCriteria(Option(filterByCurrency), None, None, None)

      val filterByCreatedAt = RoundUpSavingsCriteria(createdAt = Option(CriteriaField("", LocalDateTime.of(2019, 3, 1, 0, 0, 0), MatchTypes.LesserOrEqual)))
      val result4 = roundUpSavingsDao.getSavingOptionsByCriteria(Option(filterByCreatedAt), None, None, None)

      val filterByStatus = RoundUpSavingsCriteria(isActive = Option(CriteriaField("", true)))
      val result5 = roundUpSavingsDao.getSavingOptionsByCriteria(Option(filterByStatus), None, None, None)
    ...
    "count filtered RoundUpSaving records in getSavingOptionsByCriteria" in {
      val result1 = roundUpSavingsDao.countSavingOptionsByCriteria(None)

      val filterById = RoundUpSavingsCriteria(id = Option(CriteriaField("", 3)))
      val result2 = roundUpSavingsDao.countSavingOptionsByCriteria(Option(filterById))

      val filterByStatus = RoundUpSavingsCriteria(isActive = Option(CriteriaField("", true)))
      val result3 = roundUpSavingsDao.countSavingOptionsByCriteria(Option(filterByStatus))

      val filterByCurrentAmt = RoundUpSavingsCriteria(currentAmount = Option(CriteriaField("", BigDecimal(9000), MatchTypes.LesserOrEqual)))
      val result4 = roundUpSavingsDao.countSavingOptionsByCriteria(Option(filterByCurrentAmt))
    ...
    "get RoundUpSaving records arranged in specific order in getSavingOptionsByCriteria" in {
      val result1 = roundUpSavingsDao.getSavingOptionsByCriteria(None, Option(OrderingSet(Ordering(RoundUpSavingsSqlDao.cId, Ordering.DESC))), None, None)
      val expected1 = Seq(4, 3, 2, 1)

      result1.right.get.map(_.id) mustBe expected1

      val result2 = roundUpSavingsDao.getSavingOptionsByCriteria(None, Option(OrderingSet(Ordering(RoundUpSavingsSqlDao.cCurrAmt, Ordering.ASC))), None, None)
      val expected2 = Seq(3, 4, 2, 1)

      result2.right.get.map(_.id) mustBe expected2

      val result3 = roundUpSavingsDao.getSavingOptionsByCriteria(
        None,
        Option(OrderingSet(
          Ordering(RoundUpSavingsSqlDao.cIsActive, Ordering.DESC),
          Ordering(RoundUpSavingsSqlDao.cCreatedAt, Ordering.ASC))), None, None)
      val expected3 = Seq(4, 1, 3, 2)

      result3.right.get.map(_.id) mustBe expected3
    }

    "get RoundUpSaving records paginated with limit and offset in getSavingOptionsByCriteria" in {
      val result1 = roundUpSavingsDao.getSavingOptionsByCriteria(None, None, limit = Option(2), None)
      val expected1 = Seq(1, 2)

      result1.right.get.map(_.id) mustBe expected1

      val result2 = roundUpSavingsDao.getSavingOptionsByCriteria(None, None, limit = Option(2), offset = Option(2))
